  ...
  static async getNextBestAction(applicationId: string) {
    ...
    if (mismatchDoc) {
      return {
        ...
        description:
          'DPR cites 12,500 sq ft while Master Site Plan Layout shows 10,000 sq ft. Resolve this prior to submitting CTE application to CECB.',
    ...
    const criticalPending = application.approvals.find(
      (a) => a.isCriticalPath && (a.status === 'NOT_STARTED' || a.status === 'PENDING_DOCS')
    );
    if (criticalPending) {
      return {
        priority: 'HIGH',
    ...
    const parallelOpportunity = application.approvals.find(
      (a) => !a.isCriticalPath && a.status === 'NOT_STARTED'
    );
