...
class DocumentService {
    ...
    static async getDocumentsByApplication(applicationId) {
        return client_1.default.document.findMany({
            where: { applicationId },
            include: {
                approval: {
    ...
    static async analyzeDocument(documentId) {
        ...
        const documentMismatches = auditResults.mismatches.filter((m) => m.conflictingDocType === document.docType || document.docType === 'SITE_PLAN' || document.docType === 'DPR');
        return {
            document,
     ...
     */
    static async runConsistencyAudit(applicationId) {
        ...
        const sitePlanDoc = documents.find((d) => d.docType === 'SITE_PLAN' || d.name.toLowerCase().includes('site layout') || d.name.toLowerCase().includes('building plan'));
        const powerDoc = documents.find((d) => d.docType === 'POWER_LOAD' || d.name.toLowerCase().includes('power') || d.name.toLowerCase().includes('electrical'));
        const fireDoc = documents.find((d) => d.docType === 'FIRE_SAFETY' || d.name.toLowerCase().includes('fire'));
        let sitePlanMeta = {};
        let dprMeta = {};
        try {
            if (sitePlanDoc?.extractedMetadataJson)
                sitePlanMeta = JSON.parse(sitePlanDoc.extractedMetadataJson);
            if (dprDoc?.extractedMetadataJson)
                dprMeta = JSON.parse(dprDoc.extractedMetadataJson);
        }
        catch (e) {
        ...
        if (dprDoc && sitePlanDoc && dprArea !== sitePlanArea) {
            mismatches.push({
                ...
                severity: 'HIGH',
                impactDescription: 'Critical discrepancy detected: DPR cites 12,500 sq ft built-up area while Master Site Plan Layout shows 10,000 sq ft. This will trigger a statutory query or rejection during CECB Consent to Establish and DISH Factory Plan scrutiny.',
                recommendedAction: 'Align Section 4.2 of Detailed Project Report (DPR) with Architectural Drawing Rev-3 before submitting CTE application.',
            });
            // Update document records with mismatch
            await client_1.default.document.update({
            ...
            const existingRisk = await client_1.default.riskItem.findFirst({
                where: {
                    applicationId,
            ...
            if (!existingRisk) {
                await client_1.default.riskItem.create({
                    data: {
                        applicationId,
                        riskType: 'DOCUMENT_INCONSISTENCY',
                        severity: 'HIGH',
                        title: 'Area Mismatch: Site Layout Plan vs Detailed Project Report (DPR)',
                        description: `DPR cites ${dprArea.toLocaleString()} sq.ft built-up area while Site Layout drawing shows ${sitePlanArea.toLocaleString()} sq.ft. Difference of ${Math.abs(dprArea - sitePlanArea)} sq.ft detected.`,
    ...
    static async resolveMismatch(documentId, resolutionComment, userId) {
        const document = await client_1.default.document.findUnique({
            where: { id: documentId },
        });
        if (!document)
        ...
        await client_1.default.riskItem.updateMany({
            where: {
